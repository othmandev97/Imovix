...
class VideoProvider { 
  ...
  public static function getEnityVideo($con, $entityId, $username){
    $query = $con->prepare("SELECT videoId FROM videoProgress INNER JOIN videos 
      ON videoProgress.videoId = videos.Id WHERE videos.entityId = :entityId 
      AND videoProgress.username = :username ORDER BY videoProgress.dateModified 
      DESC LIMIT 1
    ");
    ...
    if($query->rowCount() == 0){
      $query=$con->prepare("SELECT id FROM videos WHERE entityId= :entityId ORDER BY
